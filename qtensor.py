...
class Qt:
   ...
   def genNslc(self):
      if self.nslc == None: self.nslc = numpy.prod(self.maxslc) 
      return 0
...
class qtensor:
   ...
   def diagH(self,*args):
      ...
      qt.fromQsyms(self.rank,self.qsyms,self.ndims)
      for idx in self.idx_allowed:
	 ioff  = self.ioff_allowed[idx]
	 ndim  = self.ndim_allowed[idx]         
	 dims  = self.dims_allowed[idx]
	 ...
	 qt.value[ioff_new:ioff_new+ndim] = tmp.reshape(ndim)
      return qt
   ...
   def merge(self,groups):
	 ...
	 #idlst_new.append(idlst_prod)
      ...
      qt.fromQsyms(rank,qsyms_new,ndims_new)#idlst_new)
   ...
   def transpose(self,*args):
      ...
      qt = qtensor(status)
      qt.fromQsyms(rank,qsyms,ndims)#,idlst)
      for idx in self.idx_allowed:
	 ioff  = self.ioff_allowed[idx]
	 ndim  = self.ndim_allowed[idx]         
	 dims  = self.dims_allowed[idx]
   ...
   def projectionNMs(self,qsyms0):
      qt = qtensor(self.status)
      qt.fromQsyms(self.rank,qsyms0,self.ndims)#,self.idlst)
   ...
   #@profile
   def tensordotCAL(self,qt1,qt2,thresh=1.e-20):
      self.value = numpy.zeros(self.size_allowed)
...
#
#@profile
def tensordot(qt1,qt2,axes=None):
   debug = False
   ...
   if rank == 0:
      ...
      qt = qtensor(exts12)
      # Generation of final information
      qt.fromQsyms(rank,extqsyms12,extndims12)#,extidlst12)
      if debug:
